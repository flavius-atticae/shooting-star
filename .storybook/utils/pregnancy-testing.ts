...
export const testQuebecFormats = (input: string, type: "postal_code" | "phone" | "health_card") => {
  ...
  return {
    ...
    example:
      type === "postal_code" ? "H1A 0A1" : type === "phone" ? "(514) 123-4567" : "DOUM12345678",
  };
};
...
export const runPregnancySafeTests = async (element: HTMLElement) => {
  ...
  if (badTouchTargets.length > 0) {
    results.recommendations.push(
      `Increase touch target size for ${badTouchTargets.length} elements to at least 44x44px`,
    );
  }
  ...
  if (!results.motion_safety.is_motion_safe) {
    results.recommendations.push(
      "Consider reduced motion preferences for pregnancy nausea prevention",
    );
  }
