...
test.describe('Marie - First Pregnancy Journey', () => {
  test('Marie discovers prenatal yoga services', async ({ mariePersona }) => {
    ...
    // Marie would look for prenatal-specific information
    // Note: These selectors will be updated as the site develops
...
test.describe('Sophie - Experienced Mother Journey', () => {
  ...
  test('Sophie manages multiple bookings efficiently', async ({ sophiePersona }) => {
    ...
    const navigationElements = page.locator('nav a');
...
test.describe('Alexandra - High-Risk Pregnancy Journey', () => {
  test('Alexandra accesses specialized care information', async ({ alexandraPersona }) => {
    ...
    console.log(`Testing as ${profile.name} (${profile.stage}, ${profile.gestationWeeks} weeks, high-risk)`);
    ...
    // Verify English content is available
    // Note: This will be implemented as the site develops bilingual support
