...
test.describe("Comprehensive User Journey Testing", () => {
  ...
  test("Sophie efficiently manages postpartum class schedule", async ({
    ...
    const navigationLinks = page.locator("nav a:visible");
    ...
    console.log(
      `✓ Sophie's efficient journey completed (${profile.previousBirths} previous births)`,
    );
  });

  test("Alexandra safely navigates with high-risk pregnancy needs", async ({
    alexandraPersona,
  }) => {
    ...
    console.log(
      `✓ Alexandra's safe navigation completed (${profile.gestationWeeks} weeks, high-risk)`,
    );
  });
});
...
test.describe("Error Handling and Recovery", () => {
  test("handles pregnancy brain fog form errors gracefully", async ({
    ...
    const contactForm = page
      .locator("form")
      .filter({ has: page.getByLabel("MESSAGE") })
      .first();

    // Submit invalid data and assert runtime validation feedback is shown
    await contactForm.getByLabel("NOM").fill("Marie");
    await contactForm.getByLabel("EMAIL").fill("invalid-email");
    await contactForm.getByLabel("MESSAGE").fill("Besoin d'aide rapidement.");
    ...
    await contactForm.getByRole("button", { name: /ENVOYER/i }).click();
