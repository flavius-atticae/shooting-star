...
const PREGNANCY_UNSAFE_COLORS = [
  ...
  "#990000", // Medical reds
  "#ff6b35",
  "#ff8c42", // Emergency oranges
  "#ffff00",
  "#ffff33", // Harsh yellows
] as const;
 ...
 */
// biome-ignore lint/complexity/noStaticOnlyClass: utility class shared across test files via export
class PregnancySafeTestUtils {
  ...
  static validateTouchTarget(
    ...
    const cssWidth = parseInt(computedStyle.width, 10) || 0;
    const cssHeight = parseInt(computedStyle.height, 10) || 0;

    // Check inline styles as fallback
    const inlineWidth = parseInt(element.style.width, 10) || 0;
    const inlineHeight = parseInt(element.style.height, 10) || 0;
    ...
    try {
      ...
      const message =
        `❌ ${context} width (${finalWidth}px) is smaller than pregnancy-safe minimum (${minSize}px).\n` +
    ...
    try {
      ...
      const message =
        `❌ ${context} height (${finalHeight}px) is smaller than pregnancy-safe minimum (${minSize}px).\n` +
        `   🤰 Pregnant users need larger touch targets due to:\n` +
        `   • Swollen fingers during pregnancy\n` +
        `   • Decreased dexterity\n` +
        `   • Fatigue affecting fine motor control\n` +
    ...
    if (finalWidth < recommendedSize || finalHeight < recommendedSize) {
      if (process.env.NODE_ENV === "development") {
        ...
        console.warn(
          `   Recommended: ${recommendedSize}x${recommendedSize}px for pregnancy comfort`,
        );
      }
    } else if (logDetails) {
  ...
  static validateSpacing(
    ...
    try {
      ...
      const message =
        `❌ ${context} spacing (${actualSpacing}px) is insufficient for pregnancy users.\n` +
  ...
  static validateContrast(
    foreground: string,
    background: string,
    minRatio = PREGNANCY_CONSTANTS.CONTRAST_RATIO_MIN,
  ) {
    const ratio = PregnancySafeTestUtils.getContrastRatio(foreground, background);
  ...
  static validatePregnancySafeColor(
    ...
    if (isUnsafe) {
      const message =
        `❌ ${context} uses pregnancy-unsafe color: ${color}\n` +
  ...
  static validatePerformanceBudgets(metrics: {
    ...
    const { bundleSize, loadTime, interactiveTime, context = "Performance check" } = metrics;
    ...
    if (bundleSize !== undefined) {
      ...
      if (bundleSize > budgets.BUNDLE_SIZE_KB) {
        const message =
          `❌ Bundle size (${bundleSize}KB) exceeds pregnancy-safe budget (${budgets.BUNDLE_SIZE_KB}KB).\n` +
    ...
    if (loadTime !== undefined) {
      ...
      if (loadTime > budgets.LOAD_TIME_MS) {
        const message =
          `❌ Load time (${loadTime}ms) exceeds pregnancy-safe budget (${budgets.LOAD_TIME_MS}ms).\n` +
    ...
    if (interactiveTime !== undefined) {
      console.log(
        `   Interactive time: ${interactiveTime}ms (budget: ${budgets.INTERACTIVE_TIME_MS}ms)`,
      );
      if (interactiveTime > budgets.INTERACTIVE_TIME_MS) {
        const message =
          `❌ Interactive time (${interactiveTime}ms) exceeds pregnancy-safe budget.\n` +
  ...
  static async testViewportSafely(
    ...
    if (failedTests.length > 0) {
      throw new Error(
        `Viewport tests failed for: ${failedTests.map((f) => f.viewport).join(", ")}`,
      );
    }
  ...
  static validatePregnancyFriendlyForm(formElement: HTMLElement) {
    ...
    inputs.forEach((input) => {
      ...
      PregnancySafeTestUtils.validateTouchTarget(element);
...
describe("Pregnancy-Safe Testing Patterns", () => {
  ...
  describe("Touch Target Validation Patterns", () => {
    it("should validate minimum touch targets for swollen fingers", () => {
      ...
      PregnancySafeTestUtils.validateTouchTarget(recommendedButton, {
        context: "Recommended size button",
      });
  ...
  describe("Motor Interaction Patterns", () => {
    ...
    it("should handle delayed interactions from pregnancy brain", async () => {
      ...
      render(
        <button type="button" onClick={handleClick} data-testid="pregnancy-brain-button">
          Submit Form
        </button>,
      );
    ...
    it("should handle form errors gracefully for pregnancy brain", async () => {
      ...
      render(
        <form onSubmit={handleSubmit} data-testid="error-form">
          ...
          <input id="email" type="email" style={{ minHeight: "44px" }} required />
      ...
      render(
        <div data-testid="french-content">
          {frenchTexts.map((text, index) => (
            // biome-ignore lint/suspicious/noArrayIndexKey: no stable key available for test text array
      ...
      const BilingualComponent = () => {
        ...
        return (
          <div data-testid="bilingual-content">
            <button type="button" onClick={() => setLanguage(language === "fr" ? "en" : "fr")}>
  ...
  describe("Accessibility Patterns", () => {
    ...
    it("should support keyboard navigation with pregnancy considerations", async () => {
      render(
        <nav data-testid="pregnancy-nav">
          <ul>
            <li>
              <a href="/cours" style={{ minHeight: "44px", display: "block", padding: "12px" }}>
                Cours
              </a>
            </li>
            <li>
              <a
                href="/ressources"
                style={{ minHeight: "44px", display: "block", padding: "12px" }}
              >
                Ressources
              </a>
            </li>
            <li>
              <a href="/contact" style={{ minHeight: "44px", display: "block", padding: "12px" }}>
                Contact
              </a>
            </li>
          </ul>
        ...
        return (
          <div data-testid="progressive-content">
            {Array.from({ length: loadedSections }, (_, i) => (
              // biome-ignore lint/suspicious/noArrayIndexKey: generated sections have no stable key
